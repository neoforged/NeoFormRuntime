...
| `--no-sources`                      | Use a pipeline that doesn't provide sources, by applying transforms on the .class files and using binary patches. Enabling this option adds new nodes in the graph and swaps out the global outputs that can be requested. Existing nodes are not altered and their outputs can still be requested. Since this mode can skip the decompilation and recompilation, it will be significantly faster. |
